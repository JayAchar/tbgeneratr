...
#' @importFrom dplyr left_join rename mutate recode_factor arrange filter distinct group_by top_n ungroup row_number lag
#' @importFrom magrittr %>%
...
converter.epiinfo <- function(adm, lab,
  ...
  if (convert_type == "smear") {
    data <- data %>%
      mutate(result = recode_factor(.data$result, 
                                   "Scanty" = "Positive",
                                   '1+' = "Positive", 
                                   '2+' = "Positive", 
                                   '3+' = "Positive")) 
