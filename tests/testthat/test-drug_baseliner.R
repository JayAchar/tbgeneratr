...
test_that("EpiInfo works", {
  ...
  expect_equal(class(result$base_inh_res), "factor")
  expect_equal(length(unique(result$APID)), nrow(result))
  expect_true(result$base_inh_res[result$APID == "XYZ1"] == "Sensitive")
  expect_equivalent(correct$base_inh_res, result$base_inh_res)
})
...
test_that("Days arg works", {
  expect_true(all(c("APID", "base_inh_res") %in% names(result)))
  expect_equal(length(unique(result$APID)), nrow(result))
  expect_true(result$base_inh_res[result$APID == "XYZ6"] == "Resistant")
})
